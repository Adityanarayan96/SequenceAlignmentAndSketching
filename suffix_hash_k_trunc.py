...
def lexicographic_first_ktrunc(array,mask,k): #Returns a vector with locations and the k-truncated suffix with that location as a  starting point
    best,idxs = [],[]
    ...
    return idxs[0],array[idxs[0]:idxs[0]+k]
    # return [array[idxs[0][i]:np.min(idxs[0][i]+k,np.size(array)-1)] for i in range(np.size(idxs[0]))]
...
#Function to compare similarities of two sketches
# def similarity(sketches_one,sketches_two,k)
#     temp = sketches_one - sketches_two;
#     return temp[np.where()]
...
def get_seq_sketches(seq,masks,k): #Sketch size is B
    seq = convert_DNA_to_numbers(seq);
    sketches = [lexicographic_first_ktrunc(seq,mask,k) for mask in masks]
    return sketches

def get_all_sketches(seqs,n_hash,k):
    seq_lens = [len(s) for s in seqs]
    masks = obtain_masks(max(seq_lens), n_hash);
    args = ((seq,masks,k) for seq in seqs)
    with Pool() as pool:
